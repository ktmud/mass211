...
- Most of the popular service categories correlate with each other, i.e. a neighborhood in high demand of one service is often in high demand of the other.
...
Child care related calls took the largest portion of the caseload, in part because that Mass 211 has a unique collaboration with Massachussets Department of [Early Education and Care](http://www.mass.gov/edu/birth-grade-12/early-education-and-care/) (EEC) ^[I have found no other states are having the same high-volume of calls related to child care.], but also because some parents frequently called to check the status of their enrollment application for EEC subsidies. ^[About 20% of child care related calls were explicitly marked as status check.] One possible measure to reduce these calls is to better inform parents the expected processing time, if there exists a set expectation.
...
Take the most requested two service categories for example, zip code areas with higher demands for Income Support (Child Care Assistance) almost always have a high demand for Housing Assistance (Kendall's tau coefficients $r_{\tau}$ = `r cor.res.tau`, p < 0.000).
